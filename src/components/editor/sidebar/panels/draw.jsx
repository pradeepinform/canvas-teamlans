...
import { brushSizes, drawingPanelColorPresets } from "@/options";
...
function DrawingPanel() {
  ...
  useEffect(() => {
    if (canvas) {
      toggleDrawingMode(canvas, isDrawingMode, drawingColor, brushWidth, drawingOpacity / 100);
    }
  }, [canvas, isDrawingMode, drawingColor, brushWidth, drawingOpacity]);


  const handleEraserToggle =() => {
  const isErasing = !currentEraserState;  // toggle eraser state boolean
  toggleEraseMode(canvasInstance, isErasing, currentBrushColor, 20);
  setCurrentEraserState(isErasing);
}
  ...
  const handleToggleDrawingMode = () => {
    ...
    toggleDrawingMode(canvas, newMode, drawingColor, brushWidth, drawingOpacity / 100);
  };
  ...
  const handleToggleErasing = () => {
    ...
    if (newErasing) {
      toggleEraseMode(canvas, true, drawingColor, brushWidth * 2);
    } else {
      toggleDrawingMode(canvas, true, drawingColor, brushWidth, drawingOpacity / 100);
    }
  };
